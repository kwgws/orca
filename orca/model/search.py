    ...
    async def add_document(self, document: Document, *, session: AsyncSession) -> None:
        ...
        Args:
            ...
            session (AsyncSession, optional): An active asynchronous database
                ...
                own session.
        """
        log.debug("✨ Adding Document <%s> to Search <%s>", document.guid, self.guid)
        await session.refresh(self, ["documents"])

        if document in self.documents:
            log.warning(
                "🚧 Tried adding duplicate Document <%s> to Search <%s>",
                document.guid,
                self.guid,
            )
            return
    ...
    async def add_megadoc(
        ...
        Returns:
            The newly created `Megadoc` instance, or the extant `Megadoc` with
                this same filetype.
        """
        log.debug("✨ Adding Megadoc of type '%s' to Search <%s>", filetype, self.guid)
        for megadoc in self.megadocs:
            if megadoc.filetype.lower() == filetype.lower():
                log.warning(
                    "🚧 Tried adding Megadoc of duplicate type '%s' to Search <%s>",
                    filetype,
                    self.guid,
                )
                return megadoc
