...
def _range_check(name: str, value: float, lo: float, hi: float) -> None:
    """Raise if ``value`` < ``lo`` or > ``hi``."""
    if not lo <= value <= hi:
        raise ValueError(
            f"LLM configuration error: {name}={value!r} must be in the range [{lo}, {hi}]."
        )
...
class LLMConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if bad:
            raise TypeError(
                f"LLM configuration error: Stop tokens must be non-empty strings: {bad!r}"
            )

    def to_request_dict(self) -> dict[str, Any]:
        """Return a dict ready for API use."""
        data = {
            "model": self.model,
...
class LLMRegistry(MutableMapping[str, LLMConfig]):
    """Dict-like mapping from ``alias`` to :class:`LLMConfig`."""
