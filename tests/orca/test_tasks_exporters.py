...
from orca.model import Document, Megadoc, Search
from orca.tasks import create_index, create_megadoc, create_search, import_documents
...
async def test_search(session, tmp_path):
    ...
    search01: Search = await create_search(
        "Hello", index_path=tmp_path / "index", session=session
    )
    for doc in documents:
        assert doc in search01.documents

    search02: Search = await create_search(
        "Hella~1", index_path=tmp_path / "index", session=session
    )
    for doc in documents:
        assert doc in search02.documents

    megadoc = await create_megadoc(
        ".txt", search01, data_path=tmp_path, session=session
    )
    assert isinstance(megadoc, Megadoc)
    md_path = tmp_path / megadoc.path
    assert md_path.is_file()
    md_path.rename(Path.cwd() / md_path.name)

    megadoc = await create_megadoc(
        ".docx", search02, data_path=tmp_path, session=session
    )
    assert isinstance(megadoc, Megadoc)
    md_path = tmp_path / megadoc.path
    assert md_path.is_file()
